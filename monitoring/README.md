...
the canonical metric definitions Qlik and FastAPI
both consume, not redefined ad hoc in either place:

| view | metric |
|---|---|
| `vw_IngestionHealth` | success/failure rate, rows loaded/rejected per source |
| `vw_MatchRateByStage` | match/broken/missing rate per lifecycle stage |
| `vw_BreakAgingDistribution` | break count by escalation tier, open vs. resolved |
| `vw_InvoiceDiscrepancyRate` | discrepancy rate and net $ impact per venue |
...
break/materiality thresholds that flag something as a break at all** — a disclosed distinction: materiality decides what counts
...
|---|---|---|---:|
| `CRITICAL_AGED_BREAK` | real `TIER4_CRITICAL_AGED` (14+ days open) — reused, not reinvented | critical | 254 |
| `MATERIAL_INVOICE_DISCREPANCY` | \|delta\| > $10 **or** > 50% of expected fee — notably higher than $0.01/10% "is it broken" bar | warning | 438 |
