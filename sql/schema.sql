...
-- one row per ingestion run (ingestion/run_pipeline.py), tracking
...
-- matching engine output (reconciliation/matching_engine.py).
...
-- industry-grounded root-cause labels (root_cause/taxonomy.py),
...
-- invoice reconciliation (invoice_recon/generate_invoice.py).
...
-- multi-day rolling reconciliation with break aging
...
-- immutable, append-only audit log -- architecturally enforced
...
-- alerts triggered by monitoring/alert_rules.py, scanning the
